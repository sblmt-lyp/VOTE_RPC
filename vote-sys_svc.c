...
static void vote_prog_1(struct svc_req *rqstp, SVCXPRT *transp);

static void
vote_prog_1(struct svc_req *rqstp, SVCXPRT *transp)
{
	union {
	...
	case changepassword:
		xdr_argument = (xdrproc_t) xdr_voter_pack;
...
main( int argc, char* argv[] )
{
	...
	if (getsockname(0, (struct sockaddr *)&saddr, &asize) == 0) {
		...
		_rpcpmstart = 1;
		proto = 0;
		openlog("vote-sys", LOG_PID, LOG_DAEMON);
	} else {
#ifndef RPC_SVC_FG
		int size;
		int pid, i;
...
#endif
		sock = RPC_ANYSOCK;
		(void) pmap_unset(VOTE_PROG, VOTE_VER);
	}
	...
	if ((_rpcfdtype == 0) || (_rpcfdtype == SOCK_DGRAM)) {
		...
		if (!_rpcpmstart)
			proto = IPPROTO_UDP;
		if (!svc_register(transp, VOTE_PROG, VOTE_VER, vote_prog_1, proto)) {
			_msgout("unable to register (VOTE_PROG, VOTE_VER, udp).");
			exit(1);
		}
	}
	...
	if ((_rpcfdtype == 0) || (_rpcfdtype == SOCK_STREAM)) {
		...
		if (!_rpcpmstart)
			proto = IPPROTO_TCP;
		if (!svc_register(transp, VOTE_PROG, VOTE_VER, vote_prog_1, proto)) {
			_msgout("unable to register (VOTE_PROG, VOTE_VER, tcp).");
			exit(1);
		}
	}
